...
History: PJN / 11-01-2000 1. Added some asserts to HasGotSubEntries
         ...
         PJN / 16-02-2002 1. Updated copyright message
                          ...
                          after the refresh occurs. Thanks to John No? for this fix.
                          4. Fixed a problem where the custom draw icons for network nodes were not being drawn in the correct
                          positions when scrollbars were present in the control. Again thanks to John No? for this fix.
                          ...
                          Thanks to Franz Fackelmann and John No? for spotting this problem.
         ...
         PJN / 10-06-2003 1. Custom draw is now not used for items underneath Network Neighborhood. This means that some items there will
                          ...
                          a completely new solution using the shell API's. Thanks to John No? for reporting a custom draw bug in this
         ...
         PJN / 13-12-2008 1. Optimized the memory usage by removing the m_bFileItem member variable from CFileTreeCtrlItemInfo. Instead
                          ...
                          Michael M?je for prompting this update.
         PJN / 08-01-2009 1. Updated copyright details.
                          ...
                          bug.
         PJN / 06-04-2013 1. Updated copyright details.
                          2. Updated the code to compile cleanly on VC 2010 and later
                          3. Reworked CFileTreeCtrl::AddMultipleFileNameMasks to use CString::Tokenize
                          4. Removed old Win9x code path from CShareEnumerator class.
                          5. Reworked the CSystemImageList class to remove the defunct Win9x code path.
                          6. Removed the static instances of CSystemImageList and CShareEnumerator. These changes along with
                          the Win9x changes means that the code no longer supports running on Win9x.
                          7. Reworked the code so that calling SetRootFolder on a UNC server name now works. The issue was that a 
                          path of the form "\\MACHINENAME" is not a true file system path because only paths of the form 
                          "\\MACHINENAME\SHARE" allow file enumeration. The code now explicitly checks the path and instead enumerates 
                          the network resources below the UNC server name even when SetRootFolder has been called. Thanks to Jan 
                          Moorman for reporting this issue.
         PJN / 07-04-2013 1. Minor update to the code in DisplayNetworkComputer method. 
                          2. Fixed a spelling mistake in EnumNetwork method
         PJN / 06-06-2016 1. Updated copyright details.
                          2. Updated the sample project settings to more modern default values.
                          3. Updated the code to clean compile on VC 2013 - VC 2015.
                          4. Fixed a security issue in the sample app in the use of LoadLibrary on "UXTheme.dll".
                          5. Updated the dialog based sample app's main icon.
                          6. The code now gracefully handles the situation where calls to SHGetDesktopFolder or SHGetMalloc
                          fail during control initialization.
                          7. Fixed a bug where you could not drag an item to a network share.
                          8. The code now shows shortcuts using an overlay mask. This is similar to how shared folders are optionally 
                          already shown by the control with an overlay mask.
                          9. Fixed a issue where the Current User Folder was not showing with a customized icon.
                          10. Fixed an issue in the sample app where the control would fail to show some icons initially due to the
                          fact that the sample app was not initializing OLE on startup.
                          11. Added SAL annotations to all the code
...
CFileTreeCtrlItemInfo::CFileTreeCtrlItemInfo(_In_ const CFileTreeCtrlItemInfo& ItemInfo)
{
  ...
  if (ItemInfo.m_pNetResource != NULL)
  {
...
{
  if (m_pNetResource != NULL)
  {
    if (m_pNetResource->lpLocalName != NULL)
      free(m_pNetResource->lpLocalName);
    if (m_pNetResource->lpRemoteName != NULL)
      free(m_pNetResource->lpRemoteName);
    if (m_pNetResource->lpComment != NULL)
      free(m_pNetResource->lpComment);
    if (m_pNetResource->lpProvider != NULL)
...
CSystemImageList::CSystemImageList()
{
  ...
  szDrive[0] = _T('\0');
  _tsplitpath_s(szTempDir, szDrive, sizeof(szDrive)/sizeof(TCHAR), NULL, 0, NULL, 0, NULL, 0);
  size_t nLen = _tcslen(szDrive);
  if (nLen)
  {
    if (szDrive[nLen-1] != _T('\\'))
      _tcscat_s(szDrive, sizeof(szDrive)/sizeof(TCHAR), _T("\\"));

    //Attach to the system image list
    SHFILEINFO sfi;
    memset(&sfi, 0, sizeof(sfi));
    Attach(reinterpret_cast<HIMAGELIST>(SHGetFileInfo(szTempDir, 0, &sfi, sizeof(SHFILEINFO), SHGFI_SYSICONINDEX | SHGFI_SMALLICON)));
  }
}


CShareEnumerator::CShareEnumerator() : m_pNTShareInfo(NULL),
                                       m_dwShares(0)
...
void CShareEnumerator::Refresh()
{
  ...
  NET_API_STATUS nStatus = NetShareEnum(NULL, 502, reinterpret_cast<LPBYTE*>(&m_pNTShareInfo), MAX_PREFERRED_LENGTH, &dwEntriesRead, &m_dwShares, NULL);
...
#ifndef FILETREECTRL_NO_RESOURCES
//  ON_COMMAND(ID_TREEFILECTRL_PROPERTIES, &CFileTreeCtrl::OnProperties)
//  ON_UPDATE_COMMAND_UI(ID_TREEFILECTRL_PROPERTIES, &CFileTreeCtrl::OnUpdateProperties)
//  ON_COMMAND(ID_TREEFILECTRL_RENAME, &CFileTreeCtrl::OnRename)
//  ON_UPDATE_COMMAND_UI(ID_TREEFILECTRL_RENAME, &CFileTreeCtrl::OnUpdateRename)
//  ON_COMMAND(ID_TREEFILECTRL_OPEN, &CFileTreeCtrl::OnOpen)
//  ON_UPDATE_COMMAND_UI(ID_TREEFILECTRL_OPEN, &CFileTreeCtrl::OnUpdateOpen)
//  ON_COMMAND(ID_TREEFILECTRL_DELETE, &CFileTreeCtrl::OnDelete)
//  ON_UPDATE_COMMAND_UI(ID_TREEFILECTRL_DELETE, &CFileTreeCtrl::OnUpdateDelete)
//  ON_COMMAND(ID_TREEFILECTRL_CREATEFOLDER, &CFileTreeCtrl::OnCreateFolder)
//  ON_UPDATE_COMMAND_UI(ID_TREEFILECTRL_CREATEFOLDER, &CFileTreeCtrl::OnUpdateCreateFolder)
//  ON_COMMAND(ID_TREEFILECTRL_REFRESH, &CFileTreeCtrl::OnRefresh)
//  ON_COMMAND(ID_TREEFILECTRL_UPONELEVEL, &CFileTreeCtrl::OnUpOneLevel)
//  ON_UPDATE_COMMAND_UI(ID_TREEFILECTRL_UPONELEVEL, &CFileTreeCtrl::OnUpdateUpOneLevel)
//  ON_COMMAND(ID_TREEFILECTRL_BACK, &CFileTreeCtrl::OnBack)
//  ON_UPDATE_COMMAND_UI(ID_TREEFILECTRL_BACK, &CFileTreeCtrl::OnUpdateBack)
//  ON_COMMAND(ID_TREEFILECTRL_FORWARD, &CFileTreeCtrl::OnForward)
//  ON_UPDATE_COMMAND_UI(ID_TREEFILECTRL_FORWARD, &CFileTreeCtrl::OnUpdateForward)
  ON_WM_CONTEXTMENU()
  ON_WM_MOUSEMOVE()
  ON_WM_LBUTTONUP()
  ON_WM_TIMER()
  ...
  ON_WM_DESTROY()
  ON_WM_CONTEXTMENU()
  ON_NOTIFY_REFLECT_EX(NM_DBLCLK, &CFileTreeCtrl::OnDblclk)
  ON_NOTIFY_REFLECT_EX(TVN_ITEMEXPANDING, &CFileTreeCtrl::OnItemExpanding)
  ON_NOTIFY_REFLECT_EX(TVN_BEGINLABELEDIT, &CFileTreeCtrl::OnBeginLabelEdit)
  ON_NOTIFY_REFLECT_EX(TVN_ENDLABELEDIT, &CFileTreeCtrl::OnEndLabelEdit)
  ON_NOTIFY_REFLECT_EX(NM_RCLICK, &CFileTreeCtrl::OnRclick)
#ifndef FILETREECTRL_NO_RESOURCES
  ON_NOTIFY_REFLECT_EX(TVN_BEGINDRAG, &CFileTreeCtrl::OnBeginDrag)
#endif //#ifndef FILETREECTRL_NO_RESOURCES
  ON_NOTIFY_REFLECT_EX(NM_CUSTOMDRAW, &CFileTreeCtrl::OnCustomDraw)
  ON_NOTIFY_REFLECT_EX(TVN_SELCHANGED, &CFileTreeCtrl::OnSelChanged)
  ON_NOTIFY_REFLECT_EX(TVN_DELETEITEM, &CFileTreeCtrl::OnDeleteItem)
  ON_REGISTERED_MESSAGE(WM_TREEUPDATE_CHANGE, &CFileTreeCtrl::OnChange)
END_MESSAGE_MAP()
...
BOOL CFileTreeCtrl::IsWindowsVistaOrLater()
{
  //Determine if we are running on the Vista and if so, by default show the Current User folder instead of My Documents
  ...
  return VerifyVersionInfo(&osvi, VER_MAJORVERSION | VER_MINORVERSION, dwlConditionMask) ? TRUE : FALSE;
}

int CALLBACK CFileTreeCtrl::CompareByFilenameNoCase(_In_ LPARAM lParam1, _In_ LPARAM lParam2, _In_ LPARAM /*lParamSort*/)
{
  ...
  if (pItem1->m_NodeType == CFileTreeCtrlItemInfo::MyDocumentsNode && pItem2->m_NodeType != CFileTreeCtrlItemInfo::CurrentUserFolderNode) //Then "My Documents"
    return -1;
  else if (pItem1->m_NodeType == CFileTreeCtrlItemInfo::MyComputerNode && (pItem2->m_NodeType != CFileTreeCtrlItemInfo::CurrentUserFolderNode) && (pItem2->m_NodeType != CFileTreeCtrlItemInfo::MyDocumentsNode)) //Then "My Computer/This PC"
    return -1;
...
void CFileTreeCtrl::SetShowFiles(_In_ BOOL bFiles)
{ 
...
void CFileTreeCtrl::SetRootFolder(_In_ const CString& sPath)
{
  m_sRootFolder = sPath;
...
int CFileTreeCtrl::GetIconIndex(_In_ HTREEITEM hItem)
{
  TVITEM tvi;
...
int CFileTreeCtrl::GetIconIndex(_In_ const CString& sFilename)
{
  SHFILEINFO sfi;
...
int CFileTreeCtrl::GetSelIconIndex(_In_ const CString& sFilename)
{
  SHFILEINFO sfi;
  memset(&sfi, 0, sizeof(SHFILEINFO));
...
int CFileTreeCtrl::GetIconIndex(_In_ LPITEMIDLIST lpPIDL)
{
  SHFILEINFO sfi;
  memset(&sfi, 0, sizeof(SHFILEINFO));
...
int CFileTreeCtrl::GetSelIconIndex(_In_ LPITEMIDLIST lpPIDL)
{
  SHFILEINFO sfi;
  memset(&sfi, 0, sizeof(SHFILEINFO));
...
int CFileTreeCtrl::GetSelIconIndex(_In_ HTREEITEM hItem)
{
  TVITEM tvi;
...
HTREEITEM CFileTreeCtrl::FindSibling(_In_ HTREEITEM hParent, _In_ const CString& sItem) const
{
...
HTREEITEM CFileTreeCtrl::FindServersNode(_In_ HTREEITEM hFindFrom) const
{
  if (m_bShowNetwork)
  {
...
void CFileTreeCtrl::SetHasPlusButton(_In_ HTREEITEM hItem, _In_ BOOL bHavePlus)
{
...
BOOL CFileTreeCtrl::HasPlusButton(_In_ HTREEITEM hItem)
{
  TVITEM tvItem;
  tvItem.hItem = hItem;
  tvItem.mask = TVIF_HANDLE | TVIF_CHILDREN;
...
HTREEITEM CFileTreeCtrl::SetSelectedPath(_In_ const CString& sPath, _In_ BOOL bExpanded)
{
...
BOOL CFileTreeCtrl::Rename(_In_ HTREEITEM hItem)
{
  if (hItem)
  {
...
BOOL CFileTreeCtrl::ShowProperties(_In_ HTREEITEM hItem)
{
  BOOL bSuccess = FALSE;
...
CString CFileTreeCtrl::CreateNewChildFolderName(_In_ const CString& sParentFolder)
{
...
HTREEITEM CFileTreeCtrl::CreateFolder(_In_ HTREEITEM hItem, _In_ const CString& sNewFolderName)
{
  //What will be the return value from this function assume the worst
...
BOOL CFileTreeCtrl::Delete(_In_ HTREEITEM hItem)
{
  BOOL bSuccess = FALSE;
...
BOOL CFileTreeCtrl::Open(_In_ HTREEITEM hItem)
{
  BOOL bSuccess = FALSE;
...
void CFileTreeCtrl::SetDriveHideFlags(_In_ DWORD dwDriveHideFlags)
{
...
void CFileTreeCtrl::SetFileHideFlags(_In_ DWORD dwFileHideFlags)
{
...
void CFileTreeCtrl::SetFolderHideFlags(_In_ DWORD dwFolderHideFlags)
{
...
INT_PTR CFileTreeCtrl::AddMultipleFileNameMasks(_In_ const CString& sFileNameMasks)
{
  //Empty out the array
...
void CFileTreeCtrl::SetFileNameMask(_In_ INT_PTR nIndex, _In_ const CString& sFileNameMask)
{
...
void CFileTreeCtrl::AddFileNameMask(_In_ const CString& sFileNameMask)
{
...
void CFileTreeCtrl::SetCompressedColor(_In_ COLORREF rgbCompressed)
{
...
void CFileTreeCtrl::SetShowNetwork(_In_ BOOL bShowNetwork)
{
...
void CFileTreeCtrl::SetUsingDifferentColorForCompressed(_In_ BOOL bShowCompressedUsingDifferentColor)
{
...
void CFileTreeCtrl::SetUsingDifferentIconForSharedFolders(_In_ BOOL bShowSharedUsingDifferentIcon)
{
...
void CFileTreeCtrl::SetUsingDifferentColorForEncrypted(_In_ BOOL bShowEncryptedUsingDifferentColor)
{
...
void CFileTreeCtrl::SetShowFileExtensions(_In_ HideFileExtension FileExtensions)
{
...
void CFileTreeCtrl::SetNetworkItemTypes(_In_ DWORD dwTypes)
{
...
void CFileTreeCtrl::SetShowDriveLabels(_In_ BOOL bShowDriveLabels)
{
...
void CFileTreeCtrl::SetShowMyComputer(_In_ BOOL bShowMyComputer)
{
...
void CFileTreeCtrl::SetShowDesktop(_In_ BOOL bShowDesktop)
{
...
void CFileTreeCtrl::SetShowMyDocuments(_In_ BOOL bShowMyDocuments)
{
...
void CFileTreeCtrl::SetShowCurrentUserFolder(_In_ BOOL bShowCurrentUserFolder)
{
...
void CFileTreeCtrl::SetShowRootedFolder(_In_ BOOL bShowRootedFolder)
{
...
BOOL CFileTreeCtrl::CanDisplayFile(_In_ const CFileFind& find)
{
...
BOOL CFileTreeCtrl::CanDisplayFolder(_In_ const CFileFind& find)
{
  //Derived classes can decide dynamically whether or not a 
...
BOOL CFileTreeCtrl::CanDisplayNetworkItem(_In_ CFileTreeCtrlItemInfo* /*pItem*/)
{
  //Derived classes can decide dynamically whether or not a 
...
BOOL CFileTreeCtrl::CanHandleChangeNotifications(_In_ const CString& sPath)
{
...
BOOL CFileTreeCtrl::CanDisplayDrive(_In_ const CString& sDrive)
{
  //Derived classes can decide dynamically whether or not a 
...
void CFileTreeCtrl::OnRefresh() 
{
  ...
  if (m_sRootFolder.IsEmpty())
  {
    ...
    if (m_bShowDesktop && (m_pMalloc != NULL))
    {
...
HTREEITEM CFileTreeCtrl::InsertFileItem(_In_ HTREEITEM hParent, _In_ CFileTreeCtrlItemInfo* pItem, _In_ int nStateMask, _In_ int nIcon, _In_ int nSelIcon, _In_ BOOL bCheckForChildren)
{
  //Validate our parameters
...
BOOL CFileTreeCtrl::IsExpanded(_In_ HTREEITEM hItem)
{
  TVITEM tvItem;
  tvItem.hItem = hItem;
...
CString CFileTreeCtrl::GetCorrectedLabel(_In_ CFileTreeCtrlItemInfo* pItem)
{
  ...
  switch (m_FileExtensions)
  {
    case UseTheShellSetting:
    {
      ...
      szLabel[0] = _T('\0');
      if (IsFile(pItem->m_sFQPath) && GetFileTitle(pItem->m_sRelativePath, szLabel, _MAX_PATH) == 0)
      {
        pItem->m_bExtensionHidden = (sLabel.CompareNoCase(szLabel) != 0);
        sLabel = szLabel;
      }
      break;
    }
...
void CFileTreeCtrl::DisplayRootItems()
{
  ...
  if (m_bShowCurrentUserFolder && (m_pMalloc != NULL))
  {
    CFileTreeCtrlItemInfo* pItem = new CFileTreeCtrlItemInfo;
    ...
    if (SUCCEEDED(SHGetSpecialFolderLocation(NULL, CSIDL_PROFILE, &lpDocsPidl)))
    {
      ...
      if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_PROFILE, NULL, SHGFP_TYPE_CURRENT, szFolderPath)))
      {
        ...
        nIcon = GetIconIndex(szFolderPath);
        nSelIcon = GetSelIconIndex(szFolderPath);
      }
  ...
  if (m_bShowMyDocuments && (m_pMalloc != NULL))
  {
    CFileTreeCtrlItemInfo* pItem = new CFileTreeCtrlItemInfo;
  ...
  if (m_bShowMyComputer && (m_pMalloc != NULL))
  {
    CFileTreeCtrlItemInfo* pItem = new CFileTreeCtrlItemInfo;
  ...
  if (m_bShowNetwork && (m_pMalloc != NULL))
  {
    CFileTreeCtrlItemInfo* pItem = new CFileTreeCtrlItemInfo;
...
void CFileTreeCtrl::OnSelectionChanged(_In_ NMTREEVIEW* /*pNMTreeView*/, _In_ const CString& /*sPath*/)
{
...
BOOL CFileTreeCtrl::IsFile(_In_ HTREEITEM hItem)
{
  CFileTreeCtrlItemInfo* pItem = reinterpret_cast<CFileTreeCtrlItemInfo*>(GetItemData(hItem));
  AFXASSUME(pItem != NULL);
...
BOOL CFileTreeCtrl::IsFolder(_In_ HTREEITEM hItem)
{
  CFileTreeCtrlItemInfo* pItem = reinterpret_cast<CFileTreeCtrlItemInfo*>(GetItemData(hItem));
  AFXASSUME(pItem != NULL);
...
BOOL CFileTreeCtrl::IsDrive(_In_ HTREEITEM hItem)
{
  CFileTreeCtrlItemInfo* pItem = reinterpret_cast<CFileTreeCtrlItemInfo*>(GetItemData(hItem));
  AFXASSUME(pItem != NULL);
...
BOOL CFileTreeCtrl::IsNetworkShare(_In_ HTREEITEM hItem)
{
  CFileTreeCtrlItemInfo* pItem = reinterpret_cast<CFileTreeCtrlItemInfo*>(GetItemData(hItem));
  AFXASSUME(pItem != NULL);
  ...
  if ((pItem->m_NodeType == CFileTreeCtrlItemInfo::NetworkNode) && (pItem->m_pNetResource != NULL))
    bSuccess = (pItem->m_pNetResource->dwDisplayType == RESOURCEDISPLAYTYPE_SHARE);

  return bSuccess;
}

BOOL CFileTreeCtrl::DriveHasRemovableMedia(_In_ const CString& sPath)
{
...
BOOL CFileTreeCtrl::IsCompressed(_In_ HTREEITEM hItem)
{
  return IsCompressed(ItemToPath(hItem));
}

BOOL CFileTreeCtrl::IsEncrypted(_In_ HTREEITEM hItem)
{
  return IsEncrypted(ItemToPath(hItem));
}

BOOL CFileTreeCtrl::IsFile(_In_ const CString& sPath)
{
  DWORD dwAttributes = GetFileAttributes(sPath);
  return ((dwAttributes != 0xFFFFFFFF) && ((dwAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0));
}

BOOL CFileTreeCtrl::IsFolder(_In_ const CString& sPath)
{
  DWORD dwAttributes = GetFileAttributes(sPath);
  return ((dwAttributes != 0xFFFFFFFF) && (dwAttributes & FILE_ATTRIBUTE_DIRECTORY));
}

BOOL CFileTreeCtrl::IsDrive(_In_ const CString& sPath)
{
  return (sPath.GetLength() == 3 && sPath.GetAt(1) == _T(':') && sPath.GetAt(2) == _T('\\'));
}

BOOL CFileTreeCtrl::IsUNCServerName(_In_ const CString& sPath)
{
  TCHAR szDrive[_MAX_DRIVE];
  szDrive[0] = _T('\0');
  TCHAR szDir[_MAX_DIR];
  ...
  TCHAR szExt[_MAX_EXT];
  szExt[0] = _T('\0');
  _tsplitpath_s(sPath, szDrive, sizeof(szDrive)/sizeof(TCHAR), szDir, sizeof(szDir)/sizeof(TCHAR), szFname, sizeof(szFname)/sizeof(TCHAR), szExt, sizeof(szExt)/sizeof(TCHAR));
  CString sDir(szDir);
  CString sFileName(szFname);
  return (_tcslen(szDrive) == 0) && (_tcslen(szExt) == 0) && (sDir == _T("\\\\")) && (sFileName.Find(_T('\\')) == -1);
}

BOOL CFileTreeCtrl::IsCompressed(_In_ const CString& sPath)
{
...
BOOL CFileTreeCtrl::IsEncrypted(_In_ const CString& sPath)
{
...
BOOL CFileTreeCtrl::HasGotSubEntries(_In_ const CString& sDirectory)
{
  //Validate our parameters
...
void CFileTreeCtrl::CreateMonitoringThread(_In_ const CString& sPath)
{
...
UINT CFileTreeCtrl::_MonitoringThread(_In_ LPVOID pParam)
{
  //Validate our parameters
  ASSERT(pParam != NULL);
...
BOOL CFileTreeCtrl::GetSerialNumber(_In_ const CString& sDrive, _Out_ DWORD& dwSerialNumber)
{
  return GetVolumeInformation(sDrive, NULL, 0, &dwSerialNumber, NULL, NULL, NULL, 0);
}

BOOL CFileTreeCtrl::IsMediaValid(_In_ const CString& sDrive)
{
...
BOOL CFileTreeCtrl::EnumNetwork(_In_ HTREEITEM hParent)
{
  ...
  return EnumNetwork(pNetResource, hParent);
}

BOOL CFileTreeCtrl::EnumNetwork(_In_ NETRESOURCE* pParentNetResource, _In_ HTREEITEM hParent)
{
  //What will be the return value from this function
  ...
  HANDLE hEnum;      
  DWORD dwResult = WNetOpenEnum(pParentNetResource ? RESOURCE_GLOBALNET : RESOURCE_CONTEXT, m_dwNetworkItemTypes,
                                0, pParentNetResource, &hEnum);

  //Was the read sucessful
  if (dwResult != NO_ERROR)      
  {
    TRACE(_T("CFileTreeCtrl::EnumNetwork, Cannot enumerate network, Error:%u\n"), dwResult);
    return FALSE;
  } 
  ...
  if (bSuccess)
  {
      ...
      *pNewItem->m_pNetResource = lpnrDrv[i];
      ...
      else if (lpnrDrv[i].dwDisplayType == RESOURCEDISPLAYTYPE_SERVER)
      {
        ...
        if (CanDisplayNetworkItem(pNewItem))
        {
          ...
          //speed up the performance of CFileTreeCtrl when browsing domains or workgroups with a large 
    ...
    TRACE(_T("CFileTreeCtrl::EnumNetwork, Cannot complete network drive enumeration, Error:%u\n"), dwResult);

  //Clean up the enumeration handle
  DWORD dwReturn = WNetCloseEnum(hEnum);   
...
HTREEITEM CFileTreeCtrl::DisplayNetworkComputer(_In_ HTREEITEM hParent, _In_ const CString& sComputer)
{
  //Setup the item data for the new item
  ...
  pNewItem->m_pNetResource = new NETRESOURCE;
  memset(pNewItem->m_pNetResource, 0, sizeof(NETRESOURCE));
  pNewItem->m_pNetResource->dwScope = RESOURCE_GLOBALNET;
  pNewItem->m_pNetResource->lpRemoteName = _tcsdup(sComputer);
  pNewItem->m_pNetResource->dwDisplayType = RESOURCEDISPLAYTYPE_SERVER;
...
BOOL CFileTreeCtrl::DisplayShares(_In_ HTREEITEM hParent, _In_ const CString& sComputer)
{
  ...
  BOOL bSuccess = EnumNetwork(&netResource, hParent);
  sLocalComputer.ReleaseBuffer();
  return bSuccess;
}

void CFileTreeCtrl::DisplayDrives(_In_ HTREEITEM hParent, _In_ BOOL bUseSetRedraw)
{
...
BOOL CFileTreeCtrl::IsShared(_In_ const CString& sPath)
{
  //Defer all the work to the share enumerator class
  return m_SharedEnumerator.IsShared(sPath);
}

BOOL CFileTreeCtrl::IsShortcut(_In_ const CString& sFilename)
{
  TCHAR szExt[_MAX_EXT];
  szExt[0] = _T('\0');
  _tsplitpath_s(sFilename, NULL, 0, NULL, 0, NULL, 0, szExt, sizeof(szExt) / sizeof(TCHAR));

  return _tcsicmp(szExt, _T(".LNK")) == 0;
}

void CFileTreeCtrl::DisplayPath(_In_ const CString& sPath, _In_ HTREEITEM hParent, _In_ BOOL bUseSetRedraw, _In_ BOOL bDeleteExistingSiblings)
{
  ...
  if (m_bShowRootedFolder && m_sRootFolder.GetLength() && (hParent == m_hRoot))
  {
    CFileTreeCtrlItemInfo* pItem = new CFileTreeCtrlItemInfo;
  ...
  ///
  ///이 부분이 있으면 파일까지 전부다 나오게 된다. 그래서 주석처리 함. 
  ///
  /*
...
void CFileTreeCtrl::DoExpand(_In_ HTREEITEM hItem)
{
  CFileTreeCtrlItemInfo* pItem = reinterpret_cast<CFileTreeCtrlItemInfo*>(GetItemData(hItem));
  AFXASSUME(pItem != NULL);
...
HTREEITEM CFileTreeCtrl::PathToItem(_In_ const CString& sPath) const
{
  CString sSearch(sPath);
  sSearch.MakeUpper();
  int nSearchLength = sSearch.GetLength();
...
CString CFileTreeCtrl::ItemToPath(_In_ HTREEITEM hItem) const
{
  CString sPath;
  if (hItem)
  {
    CFileTreeCtrlItemInfo* pItem = reinterpret_cast<CFileTreeCtrlItemInfo*>(GetItemData(hItem));
	if(pItem==NULL)return _T("");
    AFXASSUME(pItem != NULL);
...
BOOL CFileTreeCtrl::IsDropSource(_In_ HTREEITEM hItem)
{
  return IsFile(hItem) || IsFolder(hItem);
}

HTREEITEM CFileTreeCtrl::GetDropTarget(_In_ HTREEITEM hItem)
{
...
void CFileTreeCtrl::EndDragging(_In_ BOOL bCancel)
{
  if (IsDragging())
  {
...
BOOL CFileTreeCtrl::HasChildWithText(_In_ HTREEITEM hParent, _In_ const CString& sText)
{
  HTREEITEM hChild = GetChildItem(hParent);
  while (hChild)
  {
...
HTREEITEM CFileTreeCtrl::CopyItem(_In_ HTREEITEM hItem, _In_ HTREEITEM hNewParent, _In_ HTREEITEM hAfter)
{
...
HTREEITEM CFileTreeCtrl::CopyBranch(_In_ HTREEITEM hBranch, _In_ HTREEITEM hNewParent, _In_ HTREEITEM hAfter)
{
...
void CFileTreeCtrl::SetMaxHistory(_In_ INT_PTR nMaxHistory)
{
...
CString CFileTreeCtrl::GetBackItemText(_In_ INT_PTR nBack) const
{
  //Validate our parameters
...
CString CFileTreeCtrl::GetForwardItemText(_In_ INT_PTR nForward) const
{
  //Validate our parameters
...
void CFileTreeCtrl::SetAutoRefresh(_In_ BOOL bAutoRefresh)
{
  //Since it can be touched by more than one thead
...
void CFileTreeCtrl::CollapseExpandBranch(_In_ HTREEITEM hItem, _In_ int nAction)
{
...
void CFileTreeCtrl::PreSubclassWindow() 
{
  ...
  HRESULT hr = SHGetDesktopFolder(&m_pShellFolder);
  if (FAILED(hr))
  {
    TRACE(_T("CFileTreeCtrl::PreSubclassWindow, Failed in call to SHGetDesktopFolder, Error:%08X\n"), hr);
  }
  ASSERT(m_pMalloc == NULL);
  hr = SHGetMalloc(&m_pMalloc);
  if (FAILED(hr))
  {
    TRACE(_T("CFileTreeCtrl::PreSubclassWindow, Failed in call to SHGetMalloc, Error:%08X\n"), hr);
  }
...
/*  m_NoDropCopyCursor = pApp->LoadCursor(IDR_TREEFILECTRL_NO_DROPCOPY);
  ...
  {
    TRACE(_T("CFileTreeCtrl::PreSubclassWindow, Failed in load IDR_TREEFILECTRL_NO_DROPCOPY cursor\n"));
  }
  m_DropCopyCursor = pApp->LoadCursor(IDR_TREEFILECTRL_DROPCOPY);
  if (m_DropCopyCursor == NULL)
  {
    TRACE(_T("CFileTreeCtrl::PreSubclassWindow, Failed in load IDR_TREEFILECTRL_DROPCOPY cursor\n"));
  }
  m_NoDropMoveCursor = pApp->LoadCursor(IDR_TREEFILECTRL_NO_DROPMOVE);
  if (m_NoDropMoveCursor == NULL)
  {
    TRACE(_T("CFileTreeCtrl::PreSubclassWindow, Failed in load IDR_TREEFILECTRL_NO_DROPMOVE cursor\n"));
  }
  m_DropMoveCursor = pApp->LoadStandardCursor(IDC_ARROW);
  if (m_DropMoveCursor == NULL)
  {
    TRACE(_T("CFileTreeCtrl::PreSubclassWindow, Failed in load IDC_ARROW cursor\n"));
  }*/
#endif //#ifndef FILETREECTRL_NO_RESOURCES
}
...
void DDX_FileTreeValue(_In_ CDataExchange* pDX, _In_ CFileTreeCtrl& ctrlFileTree, _Inout_ CString& sItem)
{
