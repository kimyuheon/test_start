...
void CDLT::DrainLineType(){
	int up_cnt = 0 , k = 0,osmode = 0,OldColor=0;
	CString LineType;
	...
	TCHAR Type[30];
	Adesk::Int32 sslen = 0;
	CString LT, str_LT, strResource, LTString, vvalue, CODE, SCstring, SCset, TextStyle;
	...
	if(acedGetString(TRUE, _T("\n형식입력 = 토사(T)/L~L15/U~U15/V~V15/J~J5/다이크(D~D5)/산마루(B~B5)/맹암거(M~M10)/종배수(LO~LO5)/소단(G~G5)/용수로(Y~Y18)/플륨관(F~F5): "), Type) != RTNORM){
		setSysVar(_T("TEXTSTYLE"), TextStyle);
		setSysVar(_T("OSMODE"), osmode);		
		return;
	}
	...
	for(int i = 0; i < up_cnt; i++)
	{
		...
		CString strLINFile = strPath + _T("ZDream\\ZWCADLT.LIN");
	...
	iniFile.writeINI(_T("config.ini"), _T("GEL"), _T("SCALE"), SCset);
