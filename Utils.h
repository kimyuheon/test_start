...
const double kTwoPi	  = 3.14159265358979323846 * 2.0;

const double kRad0     = 0.0;
const double kRad45    = 3.14159265358979323846 / 4.0;
const double kRad90    = 3.14159265358979323846 / 2.0;
const double kRad135   = (3.14159265358979323846 * 3.0) / 4.0;
const double kRad180   = 3.14159265358979323846;
const double kRad270   = 3.14159265358979323846 * 1.5;
const double kRad360   = 3.14159265358979323846 * 2.0;
...
struct Utils {
	...
	static double	Interpolation(const double x1, const double y1,const double x2, const double y2, const double x);
	...
	// [+] Adesk::Boolean is bool in 2021
	//static LPCTSTR      booleanToStr(Adesk::Boolean b, CString& str);
	// [-] End
	static LPCTSTR      booleanToStr(short b, CString& str);
	...
	static void				getAllDatabases(AcArray<AcDbDatabase*>& dbPtrs);
	...
	static void					setLayerLock(AcDbObjectId& idLayer, bool islocked);



	static Acad::ErrorStatus	convertGeCurveToPolyline(AcGeCurve2d* pCurve, AcDbPolyline*& pResultPoly);

	static int					GetEntColor(AcDbEntity *&pEnt);
	static void					GetTextStyleNames(CStringArray& strArray);
	static bool					GetTextStyleName(const AcDbObjectId idTextStyle, CString& strCurTextStyle);

	static void					GetLayerName(AcDbObjectId idLayer, CString& strLayerName);
	static void					GetLayerNames(CStringArray& strArray);
	static void					GetLayer(LPCTSTR lpszLayer, AcDbObjectId& id);
	static void					GetColorOfLayer(const LPCTSTR lpszLayer, int& colorIndex);
	static bool					GetCurLayerNameAndColorIndex(CString& strCurLayer, int& colorIndex);
	static COLORREF				GetRGBFromEntity( AcDbEntity *ent );
	static int					SelColor();
	static void					refreshDisplay(void);
	static void					CreateUCS(LPCTSTR lpszNewUCS, AcGePoint3d org, AcGeVector3d vx, AcGeVector3d vy);

	static void					SetConstantWidth(AcDb2dPolyline* pline, double constWidth);
	static double				GetConstantWidth(AcDb2dPolyline* pline);


	static double				getPickBox();
	static void					PyoSi(const AcGePoint3d pos);

	static bool					EntDel(AcDbEntity *pEnt);
	static bool					EntDel(AcDbObjectId& entId);
	static void					EntDel(AcDbObjectIdArray& ids);

	static bool					SetLayer(AcDbEntity *pEnt, LPCTSTR layer);
	static bool					SetLayer(AcDbObjectId& entId, LPCTSTR layer);
	static void					SetLayer(AcDbObjectIdArray& ids, LPCTSTR layer);

	static bool					SetColorIndex(AcDbEntity *pEnt, int nColorIndex);
	static bool					SetColorIndex(AcDbObjectId& entId, int nColorIndex);
	static void					SetColorIndex(AcDbObjectIdArray& ids, int nColorIndex);

	static void					MoveObject(AcDbObjectId ObjectId, AcGeVector3d MoveVector);

	static void					ss2Array(ads_name ss, AcDbObjectIdArray& ids);
	static void					GetAllEntityWithLayer(AcArray<AcRxClass*>& arrClassType, LPCTSTR layer, AcDbObjectIdArray& entities);
};
